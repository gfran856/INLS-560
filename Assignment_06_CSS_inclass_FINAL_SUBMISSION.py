import os 
import re # 6. G.F. COMMENT - here is the import regular expression code
...
def slugify(title):
    ...
    if title.lower() == "home":  # Ensure 'Home' becomes 'index.html'
        return "index.html"
    return re.sub(r'\W+', '-', title.strip().lower()) + ".html" # 10. G.F. COMMENT - here is an example of the .lower method being used.
...
def generate_nav(titles, active_title):
    ...
    nav_links = ""
    for title in titles:        # 1. G.F. COMMENT- here is a for loop
...
def create_html_file(title, titles, output_dir="build"):
    ...
    print(f"Created {filename} in the '{output_dir}' directory.")  #2.  G.F. COMMENT - here is a f-string
# Create CSS file function
def create_css_file(output_dir="build"):
    """Generate and write the style.css file based on a dictionary of styles.""" # 9. G.F. COMMENT - dictionary 
    styles = {
    ...
    * {{
    ...
    with open(css_path, 'w') as file: #3. G.F. COMMENT - here is a open() file
        file.write(css_content) # 4. G.F. COMMENT - here is file.write

    print(f"Created style.css in the '{output_dir}' directory.")

# Create the main function
def main(): # 5. G.F. COMMENT - this is the code to create the directory 
    """Main function to generate pages and styles. MUST HAVE HOME!!!"""
    titles = ["Home", "About me", "Experience", "videos"]
...
if __name__ == "__main__": # 7. G.F. COMMENT - here is a if statement 
    main()
